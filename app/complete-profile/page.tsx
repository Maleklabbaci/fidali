...
// ✅ DÉPLACÉS EN DEHORS du composant pour éviter les re-renders
...
// ✅ Composant Field DEHORS du composant principal
...
export default function CompleteProfilePage() {
  ...
  // ✅ Pré-remplir TOUS les champs avec les données du signup
  setForm(f => ({
    ...f,
    fullName: m.name || '',
    businessName: m.business_name || '',
    phone: m.phone || '',
    businessType: m.sector || '',  // sector du signup → businessType
  }))
  ...
  const checkProfile = async () => {
    try {
      ...
      if (!profile || profile.status === 'incomplete') return
  ...
  // ✅ Handlers individuels pour éviter les re-renders
  ...
  const btnActive: React.CSSProperties = {
    ...
    color: 'white', fontFamily: "'DM Sans', sans-serif", transition: 'all 0.2s',
  }
    ...
    <div style={{
      ...
      <div style={{ width: '100%', maxWidth: 480 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[1, 2, 3].map(s => (
              <div key={s} style={{
                ...
                background: s <= step ? 'linear-gradient(90deg, #9333ea, #db2777)' : 'rgba(255,255,255,0.1)',
                transition: 'all 0.3s',
              }} />
            ))}
          </div>
        ...
        <div style={{
          ...
          borderRadius: 24, padding: 28, marginBottom: 12,
        }}>
          ...
          {step === 2 && (
            <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                <Field label="Wilaya *">
                  <select style={selectStyle} value={form.wilaya} onChange={updateField('wilaya')}>
              ...
              <div style={{ display: 'flex', gap: 10, marginTop: 24 }}>
                ...
                <button onClick={() => setStep(3)} disabled={!canNext2}
                  style={{ flex: 2, ...(canNext2 ? btnActive : btnDisabled) }}>
                  Continuer →
                </button>
              </div>
            </>
